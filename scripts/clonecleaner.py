...
class CloneCleanerScript(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        with gr.Accordion("xCloneCleaner", open=enabled):
            ...
            with FormRow():
                with FormColumn(min_width=160):
                    is_enabled = gr.Checkbox(value=enabled, label="Enable xCloneCleaner")
...
def on_ui_settings():
    section=("clonecleaner", "xCloneCleaner")
    info = shared.OptionInfo("prompt_tree.yml", "xCloneCleaner prompt database path", section=section)
    shared.opts.add_option("prompt_database_path", info)
    shared.opts.add_option(
        "enable_clonecleaner_by_default",
        shared.OptionInfo(
            True,
            "enable xCloneCleaner by default",
            gr.Checkbox,
            section=section
            )
    )
